 ...
 * different threads may are printing to the stream.
 * 
...
{
    private Map<String, PrintStream> psMap=new HashMap<String, PrintStream>();
    ...
    private PrintStream getPrintStream(Thread t) {
        ...
        if (!psMap.containsKey(name)) {
            psMap.put(name, new PrintStream(baosMap.get(name)));
        }
    ...
    public void flushAll() {
        for (PrintStream ps : psMap.values()) {
            ps.flush();
        }
    }
